...
def build_prl_student_prompt(seed_examples: str, shots: int,
                              dataset_name: str, target_question: str) -> str:
    return (
        "A user is asking this math word problem and a small language model "
        "needs help solving it:\n\n"
        f"USER QUESTION:\n{target_question}\n\n"
        f"Generate exactly {shots} in-context demonstrations to help the small "
        f"language model answer the user's question correctly. The demos should "
        f"share style and operations with the user's question, while covering "
        f"variations to make the model robust.\n\n"
        "First, inside <think>...</think>, briefly reason (2-4 sentences) about:\n"
        "  - what operations and reasoning are needed for THE USER'S QUESTION above\n"
        "  - which patterns to use across your 4 demos so they help that question\n"
        "  - variations to cover related cases without copying the user's question\n\n"
        ...
        "Rules:\n"
        "- Each demo must be self-contained (no external tables/formulas).\n"
        "- Show clear arithmetic steps.\n"
        f"- End every answer with 'The answer is [number].'.\n"
        f"- Output exactly {shots} Q/A pairs inside <answer>.\n"
        "- Do NOT include the user's exact question among the demonstrations.\n"
        "- Do NOT add anything outside <think>/<answer> blocks.\n\n"
        "Candidate examples (other similar-style problems for inspiration):\n"
        f"{seed_examples}\n\n"
        "Now output:"
    )
