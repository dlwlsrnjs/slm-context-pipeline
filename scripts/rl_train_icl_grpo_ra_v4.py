...
def make_reward_fn(args):
    ...
    def reward_fn(completions, **kwargs):
        ...
        for i, (c, q_orig, g_orig, demos, ds) in enumerate(zip(
            ...
            picks = [(q_orig, g_orig)]
            # Remaining N-1 samples — strategy-aware (random / cluster / embedding)
            ...
            if pool and len(pool) >= N:
                ...
                extras = _select_n_extras(args.selection_strategy, short,
                                          q_orig, N - 1, rng)
        ...
        # ── R_judge: 7B LLM judges demo quality (math correctness × 4 demos) ──
        judge_scores = llm_judge_batched(demos_list, max_new_tokens=500)

        # Average correctness per completion (dense signal: 0/N to N/N → N+1 levels)
        rewards = []
        for i, (r_tok, r_str, r_fmt, r_judge_unit) in enumerate(zip(
                token_rewards, structure_rewards, format_rewards, judge_scores)):
            slc = slice(i * N, (i + 1) * N)
            correct = sum(
            ...
            r_judge = args.r_judge * r_judge_unit
            total = r_tok + r_str + r_fmt + r_align + r_judge
            rewards.append(total)
            print(f"[reward c{i}] tok={r_tok:+.3f} str={r_str:+.3f} fmt={r_fmt:+.3f} "
                  f"judge={r_judge:+.3f}({int(round(r_judge_unit*4))}/4 OK) "
                  f"align={r_align:+.3f}({correct}/{N}) → total={total:+.3f}",
                  flush=True)
        return rewards
