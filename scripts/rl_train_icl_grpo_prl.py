...
def make_reward_fn(args):
    ...
    def reward_fn(completions, **kwargs):
        ...
        for c in completions:
            ...
            # 3) format: 4 valid Q/A demos inside <answer>
            demos = extract_demos_from_answer(c, 4)
            n_q = demos.count("Q:")
            r_fmt = args.r_format * min(n_q, 4) / 4.0
            format_rewards.append(r_fmt)
            demos_list.append(demos)

        # Alignment: SLM answer correctness (one batch call for all completions)
        ans_prompts = [build_answer_prompt(q, d)
                       for q, d in zip(questions, demos_list)]
        answers = slm_answer_batched(ans_prompts,
                                     max_new_tokens=args.ans_max_tokens)

        rewards = []
        for ans, gold, r_tok, r_str, r_fmt in zip(
                answers, golds, token_rewards, structure_rewards, format_rewards):
            pred = extract_final_number(ans)
            r_align = args.r_alignment if numeric_match(pred, gold) else 0.0
